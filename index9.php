...
if ($_GET['token'])
{
	...
	if ($rkActivities) {
		...
		$timestamp = $rkActivities->heart_rate[1]->timestamp;
		...
		//$ts[0]=0;
		$past=0;
		$countts =  $rkActivities->heart_rate[$count-1]->timestamp; 
		$countmin = intval(date('i',$countts)) + 1 ;
		echo "tS:".$countmin;
		for ($i=1;$i<$count;$i++)
		{
